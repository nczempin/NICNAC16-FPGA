...
mkdir -p "$SCRIPT_DIR/asic_flow/runs"
...
DESIGN_DIR="$SCRIPT_DIR/asic_flow/designs/$DESIGN_NAME"
...
# Copy config file
cp "$SCRIPT_DIR/asic_flow/config.tcl" "$DESIGN_DIR/"

# Create PDK volume directory
mkdir -p "$SCRIPT_DIR/asic_flow/pdk"

# Download PDK if not already present
if [ ! -d "$SCRIPT_DIR/asic_flow/pdk/sky130A" ]; then
    echo "Downloading Sky130 PDK..."
    docker run --rm \
        -v "$SCRIPT_DIR/asic_flow/pdk:/root/.volare" \
        efabless/openlane:latest \
        volare fetch bdc9412b3e468c102d01b7cf6337be06ec6e9c9a
fi

# Fix permissions for mounted directories
chmod -R 777 "$SCRIPT_DIR/asic_flow/designs" "$SCRIPT_DIR/asic_flow/runs" "$SCRIPT_DIR/asic_flow/pdk" 2>/dev/null || true

# Run OpenLane Docker container
echo "Starting OpenLane Docker container..."
docker run --rm \
    -v "$SCRIPT_DIR/asic_flow/designs:/openlane/designs" \
    -v "$SCRIPT_DIR/asic_flow/runs:/openlane/runs" \
    -v "$SCRIPT_DIR/asic_flow/pdk:/root/.volare" \
    efabless/openlane:latest \
...
if [ $? -eq 0 ]; then
    ...
    echo "Results are available in: $SCRIPT_DIR/asic_flow/runs/"
    
    # List generated files
    echo ""
    echo "Generated files:"
    find "$SCRIPT_DIR/asic_flow/runs" -type f -name "*.gds" -o -name "*.def" -o -name "*.lef" -o -name "*synthesis.v" | head -10
else
    echo ""
    echo "❌ OpenLane flow failed!"
    echo "Check logs in: $SCRIPT_DIR/asic_flow/runs/"
    exit 1
fi